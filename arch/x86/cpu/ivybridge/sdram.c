...
static int copy_spd(struct udevice *dev, struct pei_data *peid)
{
	...
	ret = sdram_locate_spd(dev, sizeof(peid->spd_data[0]), &data);
	if (ret)
		return ret;
...
static int sdram_find(struct udevice *dev)
{
	...
	sdram_add_memory_area(info, 1 << 20, 2 << 28);
	sdram_add_memory_area(info, (2 << 28) + (2 << 20), 4 << 28);
	sdram_add_memory_area(info, (4 << 28) + (2 << 20), tseg_base);
	sdram_add_memory_area(info, 1ULL << 32, touud);
...
int dram_init(void)
{
	...
	ret = sdram_common_init(dev, pei_data, false);
	if (ret)
		return ret;
