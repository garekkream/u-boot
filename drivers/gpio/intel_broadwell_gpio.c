...
static int broadwell_gpio_direction_input(struct udevice *dev, unsigned offset)
{
	...
	setio_le32(&regs->config[priv->offset + offset], CONFA_DIR_INPUT);
				    ...
				    int value)
{
	...
	debug("%s: dev=%s, offset=%d, value=%d\n", __func__, dev->name, offset,
	      value);
	clrsetio_le32(&regs->config[priv->offset + offset], CONFA_OUTPUT_HIGH,
					   ...
					   int value)
{
	...
	clrio_le32(&regs->config[priv->offset + offset], CONFA_DIR_INPUT);
