...
static void igd_setup_panel(struct udevice *dev)
{
	...
	// TODO: remove all the if() parts
	/* Setup Digital Port Hotplug */
	reg32 = gtt_read(priv, PCH_PORT_HOTPLUG);
	if (!reg32) {
	...
	reg32 = gtt_read(priv, PCH_PP_ON_DELAYS);
	if (!reg32) {
	...
	reg32 = gtt_read(priv, PCH_PP_OFF_DELAYS);
	if (!reg32) {
...
// 	if (!oprom_is_loaded && acpi_slp_type != 3) {
		 ...
		 */
// 		gtt_write(priv, DDI_BUF_CTL_A, DDI_BUF_IS_IDLE | DDI_A_4_LANES |
...
static int broadwell_igd_ofdata_to_platdata(struct udevice *dev)
{
	...
	if (fdtdec_get_int_array(blob, node, "intel,dp-hotplug",
			plat->dp_hotplug, ARRAY_SIZE(plat->dp_hotplug)))
		return -EINVAL;
